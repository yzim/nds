...
| `verbs/` | `nds_verbs_client` | `nds_verbs_server` | Settled one-QP PostSend, PostRecv, PollCq, RDMA Read, and configured RDMA Write correctness workflow |
...
The verbs pair is a correctness example for the settled lower-layer behavior;
its error propagation and performance benchmark are not complete. The
transport pair is the current focus and demonstrates control-path QP
negotiation plus explicit launcher data-path calls using the complete transport
descriptor and queue index.
