...
struct Config {
    ...
    std::uint32_t max_wr_bytes{64U * 1024U};
    std::uint32_t max_wrs_per_window{64U};
};

nds::Result<Config> parse(int argc, char **argv) {
    Config config;
    std::string operation{"read"};
    CLI::App app{"Benchmark RDMA inside one AIV operator launch."};
    ...
    app.add_option("--in-flight", config.in_flight)->check(CLI::Range(1U, 1024U));
...
nds::Result<void> launch_benchmark(nds::AivEntrypointLauncher *launcher, nds::client::Runtime *runtime,
                                   ...
                                   std::uint32_t iterations, void *device_args, void *device_result,
                                   NdsDeviceRdmaBenchmarkResult *result) {
    if (launcher == nullptr || runtime == nullptr || device_args == nullptr || device_result == nullptr || result == nullptr)
        return nds::unexpected(nds::ErrorCode::kInvalidArgument, "AIV benchmark launch has invalid state");
    *result = {};
    ...
    if (result->status != NDS_DEVICE_BENCHMARK_SUCCESS)
        return nds::unexpected(nds::ErrorCode::kRa, "AIV device benchmark failed with status=" +
                                                         std::to_string(result->status) + ", completion_status=" +
...
int main(int argc, char **argv) {
    (void)nds::log::configure("aiv-benchmark", "stderr", "info");
    const auto config = parse(argc, argv);
    if (!config) {
        NDS_LOG_ERROR("aiv-benchmark", "options failed: {}", config.error().message);
        return EXIT_FAILURE;
    }
    nds::client::Runtime runtime;
    nds::client::Transport transport;
    if (const auto opened = runtime.open(config->runtime); !opened) {
        NDS_LOG_ERROR("aiv-benchmark", "runtime open failed: {}", opened.error().message);
        return EXIT_FAILURE;
    }
    if (const auto opened = transport.open(&runtime, config->transport, config->execution); !opened) {
        NDS_LOG_ERROR("aiv-benchmark", "transport open failed: {}", opened.error().message);
        return EXIT_FAILURE;
    }
    ...
    if (const auto received = transport.bootstrap()->receive_bytes(record.data(), record.size()); !received) {
        NDS_LOG_ERROR("aiv-benchmark", "remote-memory bootstrap failed: {}", received.error().message);
        return EXIT_FAILURE;
    }
    ...
    if (!required_bytes || !nds::benchmark::deserialize_remote_memory(record, &remote) ||
        ...
        NDS_LOG_ERROR("aiv-benchmark", "server returned incompatible remote-memory metadata");
        return EXIT_FAILURE;
    }
    auto buffer = runtime.allocate(*required_bytes);
    if (!buffer) {
        NDS_LOG_ERROR("aiv-benchmark", "NPU allocation failed: {}", buffer.error().message);
        return EXIT_FAILURE;
    }
    ...
    if (!local || !device_transport) {
        NDS_LOG_ERROR("aiv-benchmark", "device resource setup failed");
        return EXIT_FAILURE;
    }
    nds::AivEntrypointLauncher launcher;
    if (const auto loaded = launcher.load(&runtime.acl_api(), config->execution.aiv_kernel); !loaded) {
        NDS_LOG_ERROR("aiv-benchmark", "AIV launcher load failed: {}", loaded.error().message);
        return EXIT_FAILURE;
    }
    auto device_args = runtime.allocate_device_memory(sizeof(NdsDeviceRdmaBenchmarkArgs));
    auto device_result = runtime.allocate_device_memory(sizeof(NdsDeviceRdmaBenchmarkResult));
    if (!device_args || !device_result) {
        NDS_LOG_ERROR("aiv-benchmark", "device benchmark buffers failed");
        return EXIT_FAILURE;
    }
    NdsDeviceRdmaBenchmarkResult result{};
    for (std::uint32_t index = 0U; index < config->warmup; ++index) {
        if (const auto warmed = launch_benchmark(&launcher, &runtime, *device_transport, *local, remote, *config, 1U,
                                                 *device_args, *device_result, &result); !warmed) {
            NDS_LOG_ERROR("aiv-benchmark", "warmup failed: {}", warmed.error().message);
            return EXIT_FAILURE;
        }
    }
    const auto start = std::chrono::steady_clock::now();
    if (const auto measured = launch_benchmark(&launcher, &runtime, *device_transport, *local, remote, *config,
                                               config->iterations, *device_args, *device_result, &result); !measured) {
        NDS_LOG_ERROR("aiv-benchmark", "measurement failed: {}", measured.error().message);
        return EXIT_FAILURE;
    }
    const auto elapsed = std::chrono::steady_clock::now() - start;
    (void)runtime.free_device_memory(*device_args);
    ...
    std::cout << std::fixed << std::setprecision(3) << "{\"backend\":\"aiv-device-loop\",\"operation\":\""
              << nds::benchmark::operation_name(config->operation) << "\",\"bytes\":" << config->bytes
              << ",\"in_flight\":" << config->in_flight << ",\"iterations\":" << config->iterations
              << ",\"wqe_count\":" << result.wqe_count << ",\"poll_count\":" << result.poll_count
              << ",\"elapsed_ns\":" << std::chrono::duration_cast<std::chrono::nanoseconds>(elapsed).count()
              << ",\"gib_per_second\":" << static_cast<double>(result.bytes_transferred) / seconds /
