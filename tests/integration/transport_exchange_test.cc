...
TEST(TransportExchangeIntegrationTest, ExchangesQpBatchOverOneSocket) {
    int sockets[2]{};
    ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, sockets), 0);
    const std::vector<nds::transport::QpInfo> client{make_endpoint(0x1101, 0x2202), make_endpoint(0x1102, 0x2203)};
    const std::vector<nds::transport::QpInfo> server{make_endpoint(0x3303, 0x4404), make_endpoint(0x3304, 0x4405)};
...
TEST(TransportExchangeIntegrationTest, RejectsMismatchedQpBatchCount) {
    int sockets[2]{};
    ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, sockets), 0);
    const std::vector<nds::transport::QpInfo> client{make_endpoint(0x1101, 0x2202), make_endpoint(0x1102, 0x2203)};
    const std::vector<nds::transport::QpInfo> server{make_endpoint(0x3303, 0x4404)};
    std::future<nds::Result<std::vector<nds::transport::QpInfo>>> server_result = std::async(
        std::launch::async, [fd = sockets[1], server] { return nds::TcpPeerExchange{fd}.exchange_as_server(server); });
    const auto client_result = nds::TcpPeerExchange{sockets[0]}.exchange_as_client(client);
    const auto remote_result = server_result.get();
