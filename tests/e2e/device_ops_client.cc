...
int parse(int argc, char **argv, Config *config) {
    ...
    app.add_option("--ascendcl", config->connection.context.ascendcl_library)->required();
    app.add_option("--runtime", config->connection.context.runtime_library)->required();
    app.add_option("--ra", config->connection.context.ra_library)->required();
    ...
    app.add_option("--aiv-kernel", config->connection.rma.aiv_kernel);
    app.add_option("--npu-ip", config->connection.qp.local_ipv4)->required();
    app.add_option("--logical-device", config->connection.context.logical_device_id)->required();
    app.add_option("--physical-device", config->connection.context.physical_device_id)->required();
    app.add_option("--cpu-ip", config->connection.cpu_ipv4)->required();
    app.add_option("--tcp-port", config->connection.tcp_port)->required();
    try {
        app.parse(argc, argv);
    ...
    config->connection.execution = execution == "aicpu" ? nds::NpuExecutionMode::Aicpu : nds::NpuExecutionMode::Aiv;
    config->connection.qp.physical_device_id = config->connection.context.physical_device_id;
    config->connection.tcp_timeout_ms = kTimeoutMs;
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    if (const auto allocated = connection.allocate(64U, &buffer); !allocated) {
        NDS_LOG_ERROR("npu-client", "probe allocation failed: {}", allocated.error().message);
        return EXIT_FAILURE;
    }
    if (const auto registered = connection.register_memory(&buffer, &region); !registered) {
    ...
    if (config.operation == "send") {
        ...
        payload[0] = 0x5aU;
        payload[63] = 0xa5U;
        if (const auto copied = connection.copy_to_device(&buffer, payload.data(), payload.size()); !copied) {
            NDS_LOG_ERROR("npu-client", "send payload copy failed: {}", copied.error().message);
            return EXIT_FAILURE;
        }
        const std::uint8_t ready = 1U;
        if (const auto sent = connection.bootstrap()->send_bytes(&ready, sizeof(ready)); !sent) {
            NDS_LOG_ERROR("npu-client", "probe readiness failed: {}", sent.error().message);
            return EXIT_FAILURE;
        }
        if (const auto sent = nds::client::send(&connection, region, payload.size()); !sent) {
    ...
    if (config.operation == "read" || config.operation == "write") {
        ...
        if (const auto received = connection.bootstrap()->receive_bytes(&descriptor_wire, sizeof(descriptor_wire));
            !received) {
        ...
        if (config.operation == "write" &&
            !connection.copy_to_device(&buffer, payload.data(), payload.size())) {
            NDS_LOG_ERROR("npu-client", "RDMA Write payload copy failed");
            return EXIT_FAILURE;
        }
        const auto posted = config.operation == "read"
                                ? nds::client::read(&connection, region, descriptor.data.address, descriptor.data.rkey,
                                                    static_cast<std::uint32_t>(payload.size()))
                                : nds::client::write(&connection, region, descriptor.data.address, descriptor.data.rkey,
                                                     static_cast<std::uint32_t>(payload.size()));
        if (!posted) {
        ...
        while (std::chrono::steady_clock::now() < deadline) {
            ...
            const auto count = nds::client::poll(&connection, nds::CompletionQueue::Send, &completion, 1U);
        ...
        if (config.operation == "read") {
            ...
            if (!connection.copy_from_device(received.data(), buffer, received.size()) || received != payload) {
    ...
    if (const auto posted = nds::client::post_receive(&connection, region, kReceiveWrId); !posted) {
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        nds_ra_completion completion{};
        const auto count = nds::client::poll(&connection, nds::CompletionQueue::Receive, &completion, 1U);
        if (!count) {
        ...
        if (*count == 1U) {
            ...
            if (completion.status != 0 || completion.wr_id != kReceiveWrId ||
                !connection.copy_from_device(payload.data(), buffer, payload.size()) ||
