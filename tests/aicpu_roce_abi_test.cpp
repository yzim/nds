...
int main()
{
    ...
    request.runtime_stream = UINT64_C(0x123456789000);
    ...
    if (sizeof(request) != 80U || offsetof(nds_aicpu_rdma_post_request_v2, opcode) != 8U ||
        ...
        offsetof(nds_aicpu_rdma_post_request_v2, runtime_stream) != 64U ||
        request.abi_version != 4U || request.size != sizeof(request) ||
        request.opcode != NDS_AICPU_RDMA_WRITE || request.length > NDS_AICPU_ROCE_MAX_BYTES) {
        (void)std::fputs("NDS AICPU RDMA-post ABI v4 layout check failed\n", stderr);
        return 1;
    }
    return 0;
}
