...
Result<void> wait_for_fd(int fd, short events, std::uint32_t timeout_ms) {
    ...
    if (result == 0)
        return unexpected(ErrorCode::kTransport, "TCP exchange timeout");
    if (result < 0)
        return unexpected(ErrorCode::kTransport, system_error("poll"));
    if ((descriptor.revents & (POLLERR | POLLHUP | POLLNVAL)) != 0)
        return unexpected(ErrorCode::kTransport, "TCP exchange socket became unavailable");
    return {};
}
...
Result<void> TcpConnection::send(std::span<const std::byte> bytes) const {
    if (fd_ < 0)
        return unexpected(ErrorCode::kInvalidArgument, "an open TCP connection is required");
...
Result<void> TcpConnection::read_full(int fd, void *buffer, std::size_t length) {
    ...
    while (length != 0U) {
        ...
        if (result == 0)
            return unexpected(ErrorCode::kTransport, "peer closed exchange channel");
        if (result < 0) {
            if (errno == EINTR)
                continue;
...
Result<TcpListener> TcpListener::listen(const std::string &ipv4, std::uint16_t port, int backlog) {
    if (backlog <= 0)
        return unexpected(ErrorCode::kInvalidArgument, "TCP listener backlog must be positive");
    sockaddr_in address{};
    address.sin_family = AF_INET;
    address.sin_port = htons(port);
    if (inet_pton(AF_INET, ipv4.c_str(), &address.sin_addr) != 1)
        return unexpected(ErrorCode::kInvalidArgument, "invalid TCP listen IPv4 address: " + ipv4);
...
Result<TcpConnection> TcpListener::accept() const {
    if (fd_ < 0)
        return unexpected(ErrorCode::kInvalidArgument, "an open TCP listener is required");
    int peer_fd;
    do {
        peer_fd = ::accept(fd_, nullptr, nullptr);
    } while (peer_fd < 0 && errno == EINTR);
    if (peer_fd < 0)
        return unexpected(ErrorCode::kTransport, system_error("accept"));
