...
{
    if (name == "trace") level = spdlog::level::trace;
    else if (name == "debug") level = spdlog::level::debug;
    else if (name == "info") level = spdlog::level::info;
    else if (name == "warn") level = spdlog::level::warn;
    else if (name == "error") level = spdlog::level::err;
    else if (name == "critical") level = spdlog::level::critical;
    else if (name == "off") level = spdlog::level::off;
    else return false;
    return true;
}
...
bool configure(std::string_view name, std::string_view sink_name, std::string_view level_name,
               std::string &error)
{
    try {
        std::shared_ptr<spdlog::sinks::sink> sink;
        spdlog::level::level_enum level;
        if (sink_name == "stderr") sink = std::make_shared<spdlog::sinks::stderr_color_sink_mt>();
        else if (sink_name == "stdout") sink = std::make_shared<spdlog::sinks::stdout_color_sink_mt>();
        else if (sink_name == "syslog") sink = std::make_shared<spdlog::sinks::syslog_sink_mt>(std::string(name), 0, LOG_USER, true);
        else if (sink_name == "none") sink = std::make_shared<spdlog::sinks::null_sink_mt>();
        else {
