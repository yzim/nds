...
bool poll_cpu_completion(ibv_cq *cq, ibv_wc_opcode expected_opcode, std::uint32_t timeout_ms, const char *component) {
    ...
    for (std::uint32_t elapsed = 0U; elapsed < timeout_ms; ++elapsed) {
        ...
        if (count < 0 ||
            (count == 1 && (completion.status != IBV_WC_SUCCESS || completion.opcode != expected_opcode))) {
...
bool execute_storage_command(CpuStorageTransport *transport, const nds_storage_command &command,
    ...
    if (transport == nullptr || transport->qp == nullptr || transport->cq == nullptr ||
        transport->namespace_buffer == nullptr || transport->namespace_mr == nullptr ||
        transport->completion == nullptr || transport->completion_mr == nullptr) {
    ...
    nds_storage_completion response{command.request_id, NDS_STORAGE_COMPLETION_COMPLETE, NDS_STORAGE_SUCCESS,
                                    command.length};
