...
bool NpuRaContext::ring_rdma_doorbell(std::uint32_t db_index, std::uint64_t db_info) {
    int result;

    if (!initialized_ || runtime_.set_device == nullptr || runtime_.rdma_db_send == nullptr) {
        set_error("ringing an RDMA doorbell requires an initialized context and runtime doorbell ABI");
        return false;
    }
...
std::uint32_t NpuRaContext::logical_device_id() const noexcept {
    return config_.logical_device_id;
}
