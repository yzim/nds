                    ...
                    const HostRaPostRequest &request, std::string &error)
{
    ...
    NDS_LOG_INFO("npu-client", "Posted one signaled Host RA work request: opcode={} doorbell_index={} doorbell_info=0x{:x}",
    ...
    NDS_LOG_INFO("npu-client", "Submitted OPBASE RDMA doorbell on the runtime default stream.");
    error.clear();
    return true;
}
