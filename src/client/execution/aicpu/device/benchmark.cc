...
extern "C" uint32_t NdsAicpuRdmaBenchmarkImpl(const NdsDeviceRdmaBenchmarkArgs *args) {
    ...
    if (args == nullptr || args->result_address == 0U || !NdsAicpuValidQp(&args->transport.control_qp) ||
        args->transport.control_qp.qp_mode != NDS_DEVICE_QP_MODE_NORMAL || args->local_address == 0U ||
    ...
    for (uint32_t completed = 0U; completed < args->iterations;) {
        ...
        for (uint32_t request = 0U; request < request_count; ++request) {
            ...
            const uint64_t local_base = args->local_address + static_cast<uint64_t>(slot) * args->bytes;
            const uint64_t remote_base = args->remote_address + static_cast<uint64_t>(slot) * args->bytes;
            for (uint64_t chunk_offset = 0U; chunk_offset < args->bytes; chunk_offset += chunk_bytes) {
                ...
                const NdsDeviceSendWr wr{
                    ...
                    window_wr_index + 1U == window_wrs ? static_cast<uint32_t>(NDS_DEVICE_SEND_SIGNALED) : 0U,
        ...
        uint32_t idle_polls{};
        while (completed_wrs < 1U) {
            NdsDeviceWc completions[NDS_DEVICE_MAX_COMPLETIONS]{};
            int32_t poll_result = -1;
            const uint32_t poll_status = NdsAicpuPollCqImpl(&args->transport.control_qp, 1U, 1U,
