...
extern "C" uint32_t NdsAicpuRdmaSendImpl(const nds_device_transport *transport, const nds_device_transfer *transfer,
                                         nds_device_operation_result *result) {
    return post(transport, transfer, NDS_DEVICE_WR_SEND, result);
}

extern "C" uint32_t NdsAicpuRdmaRecvImpl(const nds_device_transport *transport, const nds_device_transfer *transfer,
                                         nds_device_operation_result *result) {
    if (!valid_transport(transport, transfer, result))
        return kNdsAicpuInvalidArgument;
...
extern "C" uint32_t NdsAicpuRdmaReadImpl(const nds_device_transport *transport, const nds_device_transfer *transfer,
                                         nds_device_operation_result *result) {
    return post(transport, transfer, NDS_DEVICE_WR_RDMA_READ, result);
}

extern "C" uint32_t NdsAicpuRdmaWriteImpl(const nds_device_transport *transport, const nds_device_transfer *transfer,
                                          nds_device_operation_result *result) {
