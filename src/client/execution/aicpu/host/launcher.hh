...
public:
    ...
    Result<void> load(nds_acl_api *acl, const std::string &kernel_config_path);
    Result<void> launch_and_wait(nds_device_operation_request *request, std::int32_t completion_timeout_ms);
    Result<void> launch_post_send_and_wait(nds_device_post_send_request *request, std::int32_t completion_timeout_ms);
    Result<void> launch_storage_and_wait(nds_device_storage_request *request, std::int32_t completion_timeout_ms);
    void reset() noexcept;
