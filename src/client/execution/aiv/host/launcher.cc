...
Result<void> AivEntrypointLauncher::launch_named_and_wait(std::uint64_t args_address, const char *operator_name,
                                                          std::int32_t completion_timeout_ms) {
    if (!loaded() || args_address == 0U || operator_name == nullptr || completion_timeout_ms <= 0) {
        return unexpected(ErrorCode::kInvalidArgument,
                          "NDS AIV operator launch requires a loaded binary, args address, name, and positive timeout");
    }
...
Result<void> AivEntrypointLauncher::launch_post_recv_and_wait(std::uint64_t args_address,
                                                              std::int32_t completion_timeout_ms) {
    return launch_named_and_wait(args_address, "NdsAivPostRecv", completion_timeout_ms);
}

Result<void> AivEntrypointLauncher::launch_poll_cq_and_wait(std::uint64_t args_address,
                                                            std::int32_t completion_timeout_ms) {
    return launch_named_and_wait(args_address, "NdsAivPollCq", completion_timeout_ms);
}

Result<void> AivEntrypointLauncher::launch_rdma_send_and_wait(std::uint64_t args_address,
                                                              std::int32_t completion_timeout_ms) {
    return launch_named_and_wait(args_address, "NdsAivRdmaSend", completion_timeout_ms);
}

Result<void> AivEntrypointLauncher::launch_rdma_recv_and_wait(std::uint64_t args_address,
                                                              std::int32_t completion_timeout_ms) {
    return launch_named_and_wait(args_address, "NdsAivRdmaRecv", completion_timeout_ms);
}

Result<void> AivEntrypointLauncher::launch_rdma_read_and_wait(std::uint64_t args_address,
                                                              std::int32_t completion_timeout_ms) {
    return launch_named_and_wait(args_address, "NdsAivRdmaRead", completion_timeout_ms);
}

Result<void> AivEntrypointLauncher::launch_rdma_write_and_wait(std::uint64_t args_address,
                                                               std::int32_t completion_timeout_ms) {
    return launch_named_and_wait(args_address, "NdsAivRdmaWrite", completion_timeout_ms);
}
