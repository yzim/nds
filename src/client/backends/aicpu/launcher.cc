...
Result<void> AicpuLauncher::load(const std::string &kernel_path) {
    ...
    if (stream_result != ACL_SUCCESS || stream_ == nullptr) {
        reset();
        return Error{ErrorCode::kRuntime, "AICPU launch stream creation failed: " + std::to_string(stream_result)};
    }
    return {};
}
...
Result<void> AicpuLauncher::launch(const char *kernel_name, std::uint64_t args_gm_addr) {
    if (!loaded() || kernel_name == nullptr || args_gm_addr == 0U)
        return Error{ErrorCode::kInvalidArgument, "invalid AICPU launch arguments"};
    ...
    if (inserted) {
        ...
        if (result != ACL_SUCCESS || entry->second == nullptr) {
            ...
            return Error{ErrorCode::kRuntime, "AICPU kernel entry lookup failed: " + std::string(kernel_name)};
        }
    }
    ...
    return result == ACL_SUCCESS
               ? Result<void>{}
               : Error{ErrorCode::kRuntime, "AICPU kernel launch failed: " + std::string(kernel_name)};
}

Result<void> AicpuLauncher::synchronize(std::int32_t completion_timeout_ms) {
    if (!loaded() || completion_timeout_ms <= 0)
        return Error{ErrorCode::kInvalidArgument, "invalid AICPU synchronization timeout"};
    const int result = aclrtSynchronizeStreamWithTimeout(stream_, completion_timeout_ms);
    return result == ACL_SUCCESS ? Result<void>{} : Error{ErrorCode::kRuntime, "AICPU kernel synchronization failed"};
}
