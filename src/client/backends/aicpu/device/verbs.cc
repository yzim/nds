...
int32_t provider_opcode(uint32_t opcode) {
    if (opcode == NDS_WR_SEND)
        return NDS_HNS_WR_SEND;
    if (opcode == NDS_WR_RDMA_READ)
        return NDS_HNS_WR_RDMA_READ;
    if (opcode == NDS_WR_RDMA_WRITE)
        return NDS_HNS_WR_RDMA_WRITE;
    return -1;
}
}  // namespace

extern "C" uint32_t nds_aicpu_post_send(const NdsQpDescriptor *qp, const NdsSendWr *wr, int32_t *return_value) {
    ...
    if (opcode < 0) {
        NdsAicpuSetReturnValue(return_value, NDS_OPERATION_UNSUPPORTED);
        return kNdsAicpuSuccess;
    }
    if (qp->qp_mode != NDS_QP_MODE_NORMAL) {
        NdsAicpuSetReturnValue(return_value, NDS_OPERATION_UNSUPPORTED);
        return kNdsAicpuSuccess;
    }
    auto post = reinterpret_cast<NdsHnsExpPostSendFn>(NdsAicpuResolveSymbol("ibv_exp_post_send"));
    if (post == nullptr) {
        NdsAicpuSetReturnValue(return_value, NDS_OPERATION_SYMBOL_UNAVAILABLE);
        return kNdsAicpuSuccess;
    }
    ...
    provider_wr.send_flags = (wr->flags & NDS_SEND_SIGNALED) != 0U ? static_cast<uint32_t>(NDS_HNS_SEND_SIGNALED) : 0U;
    ...
    NdsAicpuSetReturnValue(return_value, provider_result == 0 ? NDS_OPERATION_SUCCESS : NDS_OPERATION_PROVIDER_FAILED);
    return kNdsAicpuSuccess;
}

extern "C" uint32_t nds_aicpu_post_recv(const NdsQpDescriptor *qp, const NdsRecvWr *wr, int32_t *return_value) {
    if (return_value == nullptr)
        return kNdsAicpuInvalidArgument;
    if (!NdsAicpuValidQp(qp) || wr == nullptr) {
        NdsAicpuSetReturnValue(return_value, NDS_OPERATION_INVALID_ARGUMENT);
    ...
    *reinterpret_cast<uint32_t *>(queue.head_address) = producer + 1U;
    NdsAicpuBarrier();
    NdsAicpuSetReturnValue(return_value, NDS_OPERATION_SUCCESS);
    return kNdsAicpuSuccess;
}

extern "C" uint32_t nds_aicpu_poll_cq(const NdsQpDescriptor *qp, uint32_t is_send_cq_value, uint32_t max_completions,
                                      NdsWc *wc, int32_t *return_value) {
    if (return_value == nullptr)
        return kNdsAicpuInvalidArgument;
    ...
    const NdsCqDescriptor *cq = is_send_cq ? &qp->send_cq : &qp->receive_cq;
    const NdsWorkQueueDescriptor *wq = is_send_cq ? &qp->send_queue : &qp->receive_queue;
