...
Result<void> post_send(NpuRaContext *context, NpuRaQp *qp, const BackendConfig &config, std::uint64_t address,
    ...
    if (context == nullptr || qp == nullptr)
        return unexpected(ErrorCode::kInvalidArgument, "NPU backend requires a context and QP");
    if (address == 0U || length == 0U || local_key == 0U) {
        return unexpected(ErrorCode::kInvalidArgument, "Send requires registered source memory");
    }
    ...
    if (config.mode == NpuBackendMode::Aicpu) {
        if (!qp->has_ai_qp_info()) {
            return unexpected(ErrorCode::kRa, "AICPU backend requires AI-QP metadata");
        }
    ...
    if (!qp->has_ai_qp_info() || qp->ai_qp_info().data_plane_info == nullptr) {
        return unexpected(ErrorCode::kRa, "AIV backend requires AI send-queue metadata");
    }
