...
void *DeviceBuffer::data() const noexcept { return data_; }
...
std::uint64_t RegisteredRegion::length() const noexcept { return info_.size; }
bool RegisteredRegion::belongs_to(const NpuRaQp *qp) const noexcept { return qp_ == qp && handle_ != nullptr; }

Result<void> Memory::allocate(std::size_t size, DeviceBuffer *buffer) {
    if (context_ == nullptr || buffer == nullptr || buffer->context_ != nullptr || buffer->data_ != nullptr || size == 0U)
    ...
    if (!context_->allocate_device_memory(size, &buffer->data_))
        return unexpected(ErrorCode::kRuntime, context_->error().empty() ? "invalid device allocation" : context_->error());
...
Result<void> Memory::register_memory(NpuRaQp *qp, DeviceBuffer *buffer, RegisteredRegion *region) {
    if (context_ == nullptr || qp == nullptr || buffer == nullptr || buffer->context_ != context_ || buffer->data_ == nullptr ||
        region == nullptr || region->qp_ != nullptr || region->handle_ != nullptr) {
        return unexpected(ErrorCode::kInvalidArgument, "memory registration requires a runtime buffer and empty region");
    }
...
Result<void> Memory::copy_to_device(DeviceBuffer *buffer, const void *source, std::size_t size) {
    if (context_ == nullptr || buffer == nullptr || buffer->context_ != context_ || source == nullptr || size > buffer->size_)
    ...
    if (!context_->copy_host_to_device(buffer->data_, source, size))
        return unexpected(ErrorCode::kRuntime, context_->error().empty() ? "invalid host-to-device copy" : context_->error());
    return {};
}
...
Result<void> Memory::copy_from_device(void *destination, const DeviceBuffer &buffer, std::size_t size) {
    ...
    if (!context_->copy_device_to_host(destination, buffer.data_, size))
        return unexpected(ErrorCode::kRuntime, context_->error().empty() ? "invalid device-to-host copy" : context_->error());
    return {};
}

void Memory::attach(NpuRaContext *context) noexcept { context_ = context; }
