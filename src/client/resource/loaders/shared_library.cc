...
Result<SharedLibrary> SharedLibrary::open(std::string_view path) {
    if (path.empty())
        return unexpected(ErrorCode::kInvalidArgument, "shared-library path is required");
...
Result<void *> SharedLibrary::resolve(const char *name) const {
    if (handle_ == nullptr || name == nullptr)
        return unexpected(ErrorCode::kInvalidArgument, "shared-library symbol resolution requires a handle and name");
    (void)dlerror();
    void *symbol = dlsym(handle_, name);
    const char *loader_error = dlerror();
    if (loader_error != nullptr || symbol == nullptr) {
        return unexpected(ErrorCode::kRuntime, "required symbol " + std::string(name) + " is unavailable: " +
