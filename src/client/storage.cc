...
Result<void> StorageClient::open(const TransportConfig &config) {
    if (opened_)
        return unexpected(ErrorCode::kInvalidArgument, "storage client is already open");
    if (const auto result = transport_.open(config); !result)
        return unexpected(result.error());
    if (const auto result = transport_.allocate(sizeof(nds_protocol_command_wire), &command_buffer_); !result)
        return unexpected(result.error());
    if (const auto result = transport_.allocate(sizeof(nds_protocol_completion_wire), &completion_buffer_); !result)
        return unexpected(result.error());
    if (const auto result = transport_.register_memory(&command_buffer_, &command_region_); !result)
        return unexpected(result.error());
    if (const auto result = transport_.register_memory(&completion_buffer_, &completion_region_); !result)
        return unexpected(result.error());
    const auto capacity = exchange_bootstrap();
    if (!capacity)
        return unexpected(capacity.error());
    capacity_ = *capacity;
    const nds_transport_endpoint &local = transport_.local_endpoint();
...
Result<void> StorageClient::execute(std::uint16_t operation, std::uint64_t offset, DeviceBuffer *data,
    ...
    if (request_submitted_)
        return unexpected(ErrorCode::kUnsupported, "the initial storage client supports one command per transport");
    ...
    if (const auto result = transport_.register_memory(data, &data_region); !result)
        return unexpected(result.error());
    const auto data_remote = transport_.remote_region(data_region);
    if (!data_remote)
    ...
    if (const auto result =
            transport_.copy_to_device(&completion_buffer_, &completion_wire, sizeof(completion_wire));
        !result) {
        return unexpected(result.error());
    }
    ...
    if (const auto result = transport_.copy_to_device(&command_buffer_, &command_wire, sizeof(command_wire)); !result)
        return unexpected(result.error());
    if (const auto result = transport_.ready(); !result)
        return unexpected(result.error());

    request_submitted_ = true;
    if (const auto result = transport_.send(command_region_, sizeof(command_wire)); !result)
        return unexpected(result.error());
    return wait_for_completion(request_id, length);
}

Result<std::uint64_t> StorageClient::exchange_bootstrap() {
    const auto completion = transport_.remote_region(completion_region_);
    if (!completion)
    ...
    if (const auto received = transport_.bootstrap()->receive_bytes(&namespace_wire, sizeof(namespace_wire)); !received)
...
Result<void> StorageClient::wait_for_completion(std::uint64_t request_id, std::uint64_t expected_bytes) {
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        ...
        if (const auto result = transport_.copy_from_device(&wire, completion_buffer_, sizeof(wire)); !result)
            return unexpected(result.error());
