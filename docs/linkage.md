...
| NDS AICPU package | `NdsAicpuRdmaPost` from an NDS-built `libnds_aicpu_roce.json` | Built from this repository on the selected aarch64 CANN host; dynamic load at runtime | Owns a minimal one-WR RDMA-post kernel. It neither vendors nor loads HCOMM payloads. |
