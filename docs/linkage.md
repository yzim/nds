...
6. `--submission-mode aicpu` loads only the NDS-built standard-CP1 package. NDS does not build or expose a custom-process AICPU mode because CANN does not publish the RNIC mapping/import contract that it would require.
7. CANN 9.0.0 provides an AICPU loader but no public device-side RNIC post API. The NDS-owned AICPU package is a normal aarch64 DYN shared object (built by the same shared-library model as CANN's custom-kernel template), not a `bisheng -x aicpu` relocatable object. It confines the version-coupled HNS provider ABI to `src/npu_client/modes/aicpu/device/include/nds_aicpu_hns_abi.h` and dynamically resolves `libhns-rdmav25.so:ibv_exp_post_send` through `dlopen` / `dlsym` from inside the AICPU/NPU execution environment. This matches HCOMM's device-side `DlHnsFunction` path (`HcclDlopen` is a wrapper around `dlopen`) beneath `TransportDeviceIbverbs::HnsPostSend`. The host launcher does not link, load, probe, or make availability claims about that device-side provider; it only loads NDS's kernel package through ACL.
8. Completion ownership is mode-specific: the NPU host CPU explicitly uses `RaPollCq` for host submission; HCCP owns AIV/AICPU AI-QP CQs, so NDS does not call it for those modes. ACL kernel synchronization proves kernel return, not RNIC completion. The CPU verification/acknowledgment currently keeps resources alive only for the bounded integration test; it is not the final AIV/AICPU completion interface.
