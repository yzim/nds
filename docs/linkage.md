...
The device entry point contains no copied HCOMM implementation. It validates NDS's 80-byte v4 request, dynamically opens the CANN-9.0.0 HNS provider through its device-side `dlopen`/`dlsym` loader, resolves `ibv_exp_post_send`, constructs exactly one signaled RDMA Write, RDMA Read, or Send WQE, and issues the required store barrier. It then dynamically resolves CANN's already-installed AICPU-side `hrtRDMADBSend` wrapper from `libaicpu_custom.so` and attempts to submit the provider-returned `db_info` with the AI-QP's `db_index`. The host supplies its caller-created ACL stream as opaque request metadata, and the kernel passes it to the wrapper as a runtime stream. That ACL-to-runtime-handle compatibility remains explicit target-hardware validation work, not an assumed contract. This is only the minimal post/doorbell sequence; it does not adopt HCOMM's KFC dispatcher, flag buffers, peer waits, batch/split flows, rank state, retry protocol, or background event poller. The current CPU peer exports only an ordinary remote-write destination MR, so the launcher currently exercises Write. Generic hardware acceptance remains pending.
